...
const meta = {
  ...
  decorators: [
    (Story) => (
      <div className="sidebar" style={{ width: 320, maxWidth: '100%', height: 540 }}>
        <Story />
      </div>
    ),
  ],
  beforeEach() {
    useStore.setState({
      ...
      openFile: fn(async (path: string) => {
        useStore.setState({ activeFile: path });
      }),
...
export const Narrow: Story = {
  ...LongPaths,
  render: () => (
    <div style={{ display: 'flex', width: 220, height: 540 }}>
      <FileTree />
    </div>
  ),
};
export const Empty: Story = {
  beforeEach: () => {
    useStore.setState({ files: [], unstaged: [], staged: [], activeFile: null });
  },
};
export const Loading: Story = {
  beforeEach: () => {
    useStore.setState({ files: [], unstaged: [], staged: [], filesLoading: true, activeFile: null });
  },
};
export const ErrorState: Story = {
  beforeEach: () => {
    useStore.setState({ filesError: '无法读取仓库，请确认目录仍然存在。' });
  },
};
