...
export function setupDemoStore() {
  ...
  useStore.setState({
    ...
    nvim: {
      root: '/workspace/warden',
      nvimAvailable: true,
      selected: '/tmp/nvim.42.0',
      instances: [{ socket: '/tmp/nvim.42.0', cwd: '/workspace/warden', pid: 42 }],
      scannedAt: timestamp,
    },
    ...
    refresh: fn(async () => {
      useStore.setState({ lastRefreshAt: new Date().toISOString() });
    }),
    scanNvim: fn(async () => {}),
    selectNvim: fn<AppStore['selectNvim']>(async (selected) => {
      useStore.setState((s) => ({ nvim: s.nvim ? { ...s.nvim, selected } : null }));
    }),
    focusComment: fn<AppStore['focusComment']>(async (focusedCommentId) => {
      useStore.setState({ focusedCommentId });
    }),
    jumpToComment: fn<AppStore['jumpToComment']>(async (comment) => {
      useStore.setState({ activeFile: comment.filePath, focusedCommentId: comment.id, panel: 'diff' });
    }),
    ...
    deleteComment: fn<AppStore['deleteComment']>(async (id) => {
      useStore.setState((s) => ({ comments: s.comments.filter((c) => c.id !== id), selectedCommentIds: s.selectedCommentIds.filter((x) => x !== id) }));
    }),
    ...
    createTodo: fn<AppStore['createTodo']>(async (request) => {
      const todo: Todo = {
        ...request,
        body: request.body ?? '',
        branch: request.branch ?? branchOf(useStore.getState().repo, useStore.getState().root),
        id: `new-todo-${++sequence}`,
        status: 'open',
        createdAt: timestamp,
        updatedAt: timestamp,
      };
    ...
    updateTodo: fn<AppStore['updateTodo']>(async (id, patch) => {
      useStore.setState((s) => ({ todos: s.todos.map((t) => (t.id === id ? { ...t, ...patch } : t)) }));
    }),
    deleteTodo: fn<AppStore['deleteTodo']>(async (id) => {
